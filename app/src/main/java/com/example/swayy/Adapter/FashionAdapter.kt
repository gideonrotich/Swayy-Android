...
class FashionAdapter(private var mContext: Context,
                     private  var mFashion: List<Fashion>,
                     private var isFragment: Boolean = false) : RecyclerView.Adapter<FashionAdapter.ViewHolder>()
{
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): FashionAdapter.ViewHolder {
        val view  = LayoutInflater.from(mContext).inflate(R.layout.store_item_layout, parent, false)
        return FashionAdapter.ViewHolder(view)
    }

    override fun onBindViewHolder(holder: FashionAdapter.ViewHolder, position: Int) {
        val store = mFashion[position]
