...
class TravelAdapter(private var mContext: Context,
                    private  var mTravel: List<Travel>,
                    private var isFragment: Boolean = false) : RecyclerView.Adapter<TravelAdapter.ViewHolder>()
{
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): TravelAdapter.ViewHolder {
        val view  = LayoutInflater.from(mContext).inflate(R.layout.store_item_layout, parent, false)
        return TravelAdapter.ViewHolder(view)
    }

    override fun onBindViewHolder(holder: TravelAdapter.ViewHolder, position: Int) {
        val store = mTravel[position]
