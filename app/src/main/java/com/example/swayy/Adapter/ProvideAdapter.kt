...
import com.example.swayy.model.Travel
import com.example.swayy.stores.storedetailFragment

class ProvideAdapter (private var mContext: Context,
                      private  var mProvide: List<Provide>,
                      private var isFragment: Boolean = false) : RecyclerView.Adapter<ProvideAdapter.ViewHolder>()
{
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ProvideAdapter.ViewHolder {
        val view  = LayoutInflater.from(mContext).inflate(R.layout.store_item_layout, parent, false)
        return ProvideAdapter.ViewHolder(view)
    }

    override fun onBindViewHolder(holder: ProvideAdapter.ViewHolder, position: Int) {
        val store = mProvide[position]
